...
function openYouTubeVideoForTrack(trackInfo) {
  ...
  setTimeout(() => {
    ...
    const showApiResults = async (query) => {
        ...
        if (!data.items || data.items.length === 0) {
          contentContainer.innerHTML = `
            <div style="width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center; background: #121212; color: white; text-align: center; padding: 20px;">
              ...
              <button onclick="window.open('https://www.youtube.com/results?search_query=${encodedQuery}', '_blank')" 
    ...
    // Function to show search results using embed
    const showEmbedResults = (query) => {
 ...
 */
async function init() {
  console.debug("YT-Video: Initializing");
